create_paper_stats_csv = function(data, ns_dz_data, file_name) {
  owe_data_version = data |>
    summarize(value = first(data_version)) |>
    mutate(name = "OWE repository version")
  ...
  countries_in_repo = data |>
    distinct(country) |>
    arrange(country) |>
    summarize(value = paste(country, collapse = ", ")) |>
    mutate(name = "Countries in repository")
  ...
  published_since_2010 = data |>
    ...
    summarize(
      ...
      "Published since 2010, share" = label_percent(accuarcy = 1)(mean(
        since_2010
      ))
    ) |>
  ...
  number_studies_2010_broad_v_narrow = data %>%
    ...
    mutate(
      name = case_when(
        overall == 1 ~
          "Number of published studies in year >= 2010, broad group",
        overall != 1 ~
          "Number of published studies in year >= 2010, narrow group"
      )
    )
  ...
  negative_owe_share_us_2021 = data %>%
    ...
    mutate(value = label_percent(accuracy = 1)(value)) %>%
    mutate(
      name = "Share of published US studies in 2021 or before with negative OWE"
    )
  ...
  mean_median_reg_tests = tribble(
    ~name,
    ~value,
    "Null: equality of means  , 2000-2009 and 2020-2025, p-value",
    lm_p,
    "Null: equality of medians, 2000-2009 and 2020-2025, p-value",
    rq_p
  ) |>
  ...
  share_positive_since_2010 = data |>
    filter(published == 1, year >= 2010) |>
    summarize(value = mean(owe_b >= 0)) |>
    mutate(
      value = label_percent(accuracy = 0.1)(value),
      name = "Positive share of published studies since 2010"
    )

  dz_published_US_count = data |>
    filter(published == 1 & country == "US") |>
    nrow()

  dz_published_US_count_positive = data |>
    filter(published == 1 & country == "US" & owe_b >= 0) |>
    nrow()

  dz_us_pub_positive_counts = tibble(
    name = "DZ US published count",
    value = as.character(dz_published_US_count)
  ) |>
    add_row(
      name = "DZ US published positive count",
      value = as.character(dz_published_US_count_positive),
    ) |>
    add_row(
      name = "DZ US published positive share",
      value = label_percent(accuracy = 1)(
        dz_published_US_count_positive / dz_published_US_count
      )
    )
  ...
  dz_positive_count_share = ns_dz_data |>
    ...
    summarize(
      ...
      `DZ studies US published, share positive` = label_percent(
        accuracy = 1
      )(mean(dz_pos))
    ) |>
    pivot_longer(everything())
  ...
  ns_should_include_studies = ns_dz_data |>
    filter(is.na(dz_study_id) | dz_published == 1 & dz_country == "US") |>
    filter(ns_status == "NS missing") |>
    inner_join(data, join_by(dz_study_id == study_id))

  ns_should_include_studies_count_year = ns_should_include_studies |>
    mutate(name = if_else(year <= 2021, "2021 or before", "after 2021")) |>
    summarize(value = n(), .by = name) |>
    mutate(
      name = paste("NS should/would have included these studies, count,", name),
      value = as.character(value)
    )

  extra_studies_positive_count = ns_should_include_studies |>
    summarize(value = as.character(sum(dz_pos == 1))) |>
    mutate(name = "NS should/would have included these studies, positive count")

  ns_positive_share_with_extra_studies = ns_dz_data |>
    filter(ns_status != "NS missing") |>
    bind_rows(ns_should_include_studies) |>
    mutate(positive = if_else(!is.na(ns_pos), ns_pos, dz_pos)) |>
    summarize(value = label_percent(accuracy = 1)(mean(positive))) |>
    mutate(name = "NS augmented with extra studies, positive share")
  ...
  share_studies_ns_dz_overlap_positive = ns_dz_data %>%
    filter(!is.na(dz_study_id), !is.na(ns_study_id)) %>%
    summarize(value = label_percent(accuracy = 1)(mean(dz_pos))) %>%
    mutate(name = "Share of studies in NS DZ intersection with OWE positive")
  ...
  results = bind_rows(
    ...
    count_studies_all,
    countries_in_repo,
    count_studies_published,
    median_owe,
    mean_owe,
    number_studies_broad,
    median_broad,
    share_studies_more_pos,
    number_journals,
    journals_with_at_least_4,
    averaged_estimates_all,
    averaged_estimates_published,
    owe_reported_all,
    owe_reported_published,
    ...
    median_us,
    mean_us,
    number_studies_narrow,
    median_narrow,
    broad_owe_2020_share,
    narrow_owe_2020_share,
    owe_2010_broad_v_narrow,
    number_studies_2010_broad_v_narrow,
    number_studies_rr,
    number_studies_teen,
    median_teen,
    mean_teen,
    ...
    mean_rr,
    size_by_period,
    number_studies_by_decade,
    mean_median_by_decade,
    mean_median_reg_tests,
    medium_large_2010_test,
    negative_owe_share,
    negative_owe_share_us_2021,
    share_positive_since_2010,
    ns_positive_count_share,
    dz_us_pub_positive_counts,
    dz_ns_superset_count,
    ns_should_include_studies_count_year,
    extra_studies_positive_count,
