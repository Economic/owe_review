...
define_histograms <- function() {
  tribble(
    ...
    "count",
    "broad",
    "published == 1 & overall == 1",
    "percent",
    "narrow",
    "published == 1 & overall != 1",
    "percent",
    "rr",
    "published == 1 & restaurants_retail == 1",
    "percent",
    "teens",
    "published == 1 & teens == 1",
    "percent",
    "before_2010",
    "published == 1 & year < 2010",
    "percent",
    "after_2010",
    "published == 1 & year >= 2010",
    "percent"
...
make_owe_reported_plot <- function(owe_data) {
  ...
  owe_data %>%
    ...
    summarize(
      `Employment and wage\nelasticities only` = sum(1 - owe_reported),
      `OWE reported\nby authors` = sum(owe_reported),
      .by = decade
    ) %>%
...
make_owe_decade_plot <- function(owe_data, bootstraps) {
  ...
  data |>
    ...
    scale_x_continuous(
      breaks = seq(1, 4),
      labels = c("1992 - 1999", "2000 - 2009", "2010 - 2019", "2020 - 2025"),
      minor_breaks = NULL
    ) +
    guides(alpha = "none") +
    theme_minimal(base_family = "Roboto Condensed") +
