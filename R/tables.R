...
make_summary_table <- function(owe_data, file) {
  published <- owe_data %>%
    filter(published == 1) %>%
    summary_df() %>%
  ...
  table_data <- owe_data %>%
    ...
    mutate(across(
      large_negative | medium_negative | small_negative | zero_positive,
  ...
  table_data %>%
    ...
    add_header_above(c(" " = 4, "Distribution of effect sizes" = 4)) |>
    str_replace_all("\\\\\\{", "\\{") %>%
    str_replace_all("\\\\\\}", "\\}") %>%
    str_replace_all(
      "Number of studies",
...
make_dz_ns_table = function(data, file) {
  ...
  table_data |>
    ...
    column_spec(
      1:5,
      bold = if_else(table_data$dz_status == "NS status total", TRUE, FALSE)
    ) %>%
